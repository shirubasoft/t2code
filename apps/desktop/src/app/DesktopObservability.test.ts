...
describe("DesktopObservability", () => {
  ...
  it.effect("keeps main process logs local with a configured logs endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Log messages remain in the local trace even with an exporter configured.
  ...
  it.effect("does not export any signal when endpoints are configured in Settings", () => {
    const requests: Array<ExportedRequest> = [];
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
