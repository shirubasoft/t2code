...
#
#   irm https://raw.githubusercontent.com/shirubasoft/t2code/main/scripts/install.ps1 | iex
#
# Environment:
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "t3 $version has no release archive for win32-$arch; choose an available T2 Code release"
      }
      throw
    }
