...
#
#   curl -fsSL https://raw.githubusercontent.com/shirubasoft/t2code/main/scripts/install.sh | sh
#
# Environment:
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  if [ "$fetch_status" -eq 44 ]; then
    fail "t3 ${version} has no release archive for ${platform}-${arch}; choose an available T2 Code release"
